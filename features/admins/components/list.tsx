...
export default function AdminList({ updated }: { updated: boolean }) {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        setIsLoading(true);
        const response = await adminService.list(queryParams);
        if (isMounted) {
  ...
  }, [queryParams, updated, refetch]);

  if (isLoading && data.length === 0) {
    return <DataTableSkeleton columnCount={getColumns(() => {}).length} />;
  }
