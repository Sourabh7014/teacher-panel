...
export default function ArticleCategoryList({ updated }: { updated: boolean }) {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await articleService.categories(queryParams);
      setData((response?.categories as unknown as ArticleCategory[]) ?? []);
      if (response?.meta) {
  ...
  }, [fetchCategories, updated, localUpdated]);

  const { openModal } = useModal();

  const onCreate = () => {
    openModal(CreateCategory, {}, {}, (result) => {
      if (result) {
        setLocalUpdated((prev) => !prev);
      }
    });
  };
