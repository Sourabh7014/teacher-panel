...
export default function CategoryList({ updated }: { updated: boolean }) {
  const [data, setData] = useState<FeedbackCategory[]>([]);
  ...
  const fetchCategories = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await feedbackService.categories(queryParams);
      setData((response?.categories as FeedbackCategory[]) ?? []);
      if (response?.meta) {
