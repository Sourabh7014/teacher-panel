...
export default function CreateCategory({
  ...
  onConfirm: (data: ReportCategory | null) => void;
}) {
  ...
  async function onSubmit(values: CreateCategoryValues | EditCategoryValues) {
    try {
      setLoading(true);
      const response = await reportService.saveCategory(values, category?.id);
      console.log(response);
      if (response?.category) {
        onConfirm(response.category as ReportCategory);
      }
    } finally {
      setLoading(false);
    }
